const API_BASE = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' 
  ? 'http://localhost:4000' 
  : 'https://brainfit-narlidere-2.onrender.com';
...
async function handleFormSubmit(e) {
  ...
  try {
    // 1. Send to Custom Backend (Render)
    const backendPromise = fetch(`${API_BASE}/lead`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload)
    }).catch(err => console.error('Backend save error:', err));

    // 2. Send to Formspree (Keep existing logic)
    ...
    const [backendRes, formspreeRes] = await Promise.all([backendPromise, formspreePromise]);
