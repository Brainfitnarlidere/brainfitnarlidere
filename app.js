...
async function onFormSub(e) {
  ...
  try {
    ...
    if (sb) {
      ...
      // Mark as captured if they fill the main lead form too
...
async function onEmailCaptureSub(e) {
  e.preventDefault();
  var f = e.target;
  ...
  try {
    ...
    if (btn) { btn.disabled = false; btn.textContent = 'Hemen PDF\'i Gönder'; } 
  }
}
...
function boot() {
  ...
  if (document.getElementById('emailCaptureForm')) document.getElementById('emailCaptureForm').addEventListener('submit', onEmailCaptureSub);
  ...
  if (document.getElementById('modalClose')) document.getElementById('modalClose').onclick = function() { 
    document.getElementById('emailModal').hidden = true; 
    localStorage.setItem(EMAIL_KEY, 'true'); // Don't show again in this session/device
  };
