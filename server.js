...
async function getGoogleSheets() {
  ...
  if (!credentials || !spreadsheetId) {
    console.log('Google Sheets config missing, skipping Google Sheets sync.');
    return null;
  }
...
async function saveToGoogleSheets(sheetName, data) {
  try {
    const config = await getGoogleSheets();
    if (!config) return;

    const { sheets, spreadsheetId } = config;
    ...
    console.log(`Google Sheets'e başarıyla eklendi (${sheetName})`);
  } catch (err) {
    console.error('Google Sheets Hatası:', err.message);
  }
}
...
const allowedOrigins = [
  ...
  'https://brainfitnarlidere.com',
  'https://www.brainfitnarlidere.com'
];
...
app.use(cors({
  origin: function (origin, callback) {
    ...
    // Check if origin is in allowed list or is a subdomain of github.io (optional but safe)
